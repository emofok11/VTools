...
function stripDescriptionEditorTrailingBreak(value?: string): string {
  if (!value) return '';
  return value
    .replace(/<span\s+data-description-trailing-break="true"[^>]*>.*?<\/span>$/i, '')
    .replace(/<br\s+data-description-trailing-break="true"\s*\/?>(?:\s|&nbsp;|&#160;|&#8203;)*$/i, '');
}

// 仅在编辑器显示态补一个尾部占位换行，确保末尾空行在 contentEditable 中稳定可见。
function ensureDescriptionEditorTrailingBreak(value?: string): string {
  const sanitizedValue = stripDescriptionEditorTrailingBreak(value);
  if (!sanitizedValue) return '';
  return `${sanitizedValue}${DESCRIPTION_EDITOR_TRAILING_BREAK_HTML}`;
}
...
function descriptionHtmlToPlainText(value?: string): string {
  ...
  if (!isDescriptionRichHtml(normalizedValue)) {
...
function sanitizeDescriptionRichHtml(value?: string, fallbackColor?: string): string {
  ...
  if (!normalizedValue) return '';
  if (!isDescriptionRichHtml(normalizedValue)) {
    return descriptionPlainTextToHtml(descriptionHtmlToPlainText(normalizedValue));
  }
...
function getDescriptionNodeTextLength(node: Node): number {
  ...
  if (isDescriptionEditorTrailingBreakNode(node)) {
    return 0;
  }
...
function resolveDescriptionDomPosition(root: HTMLElement, offset: number): { node: Node; offset: number } {
  ...
  const walker = document.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT | NodeFilter.SHOW_ELEMENT,
    {
      acceptNode(node) {
        ...
        if (isDescriptionEditorTrailingBreakNode(node)) return NodeFilter.FILTER_SKIP;
  ...
  const trailingBreakNode = Array.from(root.childNodes).find(child => isDescriptionEditorTrailingBreakNode(child));
  const endOffset = trailingBreakNode
    ? Array.from(root.childNodes).indexOf(trailingBreakNode)
...
const [showAddMenu, setShowAddMenu] = useState(false); // 添加容器下拉菜单
  ...
  const captureDescriptionSnapshot = useCallback((
    ...
    const rawValue = stripDescriptionEditorTrailingBreak(valueOverride ?? editor?.innerHTML ?? textValues[fieldId] ?? '');
  ...
  const handleDescriptionSelectionChange = useCallback((fieldId: string, currentValue: string) => {
    ...
    const liveValue = sanitizeDescriptionRichHtml(stripDescriptionEditorTrailingBreak(editor.innerHTML || currentValue), fallbackColor);
    descriptionSelectionRangesRef.current[fieldId] = selection;
    setActiveDescriptionSelection({
      fieldId,
      start: selection.start,
  ...
  const handleDescriptionKeyUp = useCallback((
    ...
    // Enter / Backspace / Tab 已在 keydown 阶段程序化更新内容和光标，
    // keyup 若继续读取旧 DOM 选区，会把新光标位置覆盖回去。
  ...
  const handleDescriptionColorChange = useCallback((fieldId: string, color: string) => {
    const editor = descriptionEditorRefs.current[fieldId];
    const fallbackColor = normalizeDescriptionColor(textValues[getDescriptionColorFieldId(fieldId)]);
    const currentValue = sanitizeDescriptionRichHtml(stripDescriptionEditorTrailingBreak(editor?.innerHTML || textValues[fieldId] || ''), fallbackColor);
  ...
  const handleDescriptionBeforeInput = useCallback((fieldId: string, event: React.FormEvent<HTMLDivElement>) => {
    if (isApplyingDescriptionHistoryRef.current) {
      return;
    }

    // 拦截 Enter 产生的 insertParagraph / insertLineBreak，Enter 已在 keydown 阶段统一处理。
    // 部分浏览器（Chrome）即使 keydown 调用了 preventDefault()，仍可能触发 beforeinput。
  ...
  const handleDescriptionInput = useCallback((fieldId: string, event: React.FormEvent<HTMLDivElement>) => {
    const nativeEvent = event.nativeEvent as InputEvent;
    // 防御性跳过：仅在浏览器意外派发换行类 input 时跳过，避免覆盖 keydown 已经写入的状态。
    if (nativeEvent.inputType === 'insertParagraph' || nativeEvent.inputType === 'insertLineBreak') {
      return;
    }
    ...
    descriptionPendingSnapshotRef.current[fieldId] = null;
    const liveHtml = stripDescriptionEditorTrailingBreak(element.innerHTML);
    const tokens = descriptionHtmlToCharTokens(liveHtml, fallbackColor);
  ...
  const handleDescriptionKeyDown = useCallback((fieldId: string, event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // ===== 使用列表编辑工具处理键盘事件 =====
    // Enter / Backspace / Tab 均在 keydown 阶段拦截，统一走 handleDescriptionTextChange 流程。
    if (event.key === 'Enter' || event.key === 'Backspace' || event.key === 'Tab') {
      ...
      if (result.handled && result.newText !== undefined) {
        ...
        // Enter / Backspace / Tab 统一走 handleDescriptionTextChange 流程
...
<span className="badge-icon">☰</span>
            ...
            {/* 版本号旁边：官方/锁定标识 */}
            {template.isOfficial && (
              <span className="header-badge header-badge-official" title="官方模版">★</span>
            )}
            {template.isLocked && (
              <span className="header-badge header-badge-locked" title="模版已锁定，不可编辑">🔒</span>
            )}
          </div>
          {/* 显示当前选中的图片坑位提示 */}
          {selectedSlotId && (
...
☰ 已选中: {selectedSlotId}
        ...
        <div className="header-actions">
          <button className="btn-save" onClick={handleSave} disabled={template.isLocked}>
            {template.isLocked ? '🔒 已锁定' : '保存草稿'}
          </button>
              ...
              >
                ...
                <div className="module-content">
                  ...
                  {moduleType === 'description' && module.id.startsWith('custom-desc-') && moduleTextFields.length > 0 && (() => {
                    ...
                    const editorHtml = ensureDescriptionEditorTrailingBreak(currentValue);
                        ...
                        <div className="description-editor-shell">
                          <div
                            ref={node => {
                              ...
                              // React 18 不会在 contentEditable 元素上代理 beforeinput 事件，
                              // 因此必须用原生 addEventListener 监听，拦截浏览器在 keydown preventDefault
                              // 之后仍然触发的 insertParagraph / insertLineBreak。
                              if (node && !(node as any).__beforeInputBound) {
                                (node as any).__beforeInputBound = true;
                                node.addEventListener('beforeinput', (e: Event) => {
                            ...
                            onBeforeInput={(e) => handleDescriptionBeforeInput(descriptionField.id, e)} /* 注意：React 18 不会在 contentEditable 上代理 beforeinput，此回调不会触发 */
                            ...
                            onKeyUp={(e) => handleDescriptionKeyUp(descriptionField.id, textValues[descriptionField.id] || '', e)}
